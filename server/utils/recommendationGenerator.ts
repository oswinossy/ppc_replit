...
export async function generateDailyRecommendations(): Promise<{ total: number; countries: number; keywords: number; placements: number }> {
  ...
  for (const country of COUNTRIES) {
    try {
      ...
      const brandKeywordCount = await generateBrandKeywordRecommendationsForCountry(country, connectionUrl);
      ...
      const totalKeywordCount = keywordCount + brandKeywordCount;
      const placementCount = productPlacementCount + brandPlacementCount;

      if (totalKeywordCount > 0 || placementCount > 0) {
        totalKeywords += totalKeywordCount;
        totalPlacements += placementCount;
        countriesProcessed++;
        console.log(`[RecommendationGenerator] ${country}: ${keywordCount} SP keyword + ${brandKeywordCount} SB keyword + ${productPlacementCount} product placement + ${brandPlacementCount} brand placement recommendations`);
      }
    } catch (error) {
...
export async function generateRecommendationsForCountry(country: string): Promise<{ keywords: number; placements: number; total: number }> {
  ...
  return { keywords: totalKeywordCount, placements: placementCount, total: totalKeywordCount + placementCount };
}
...
// Generate brand keyword bid recommendations from s_brand_search_terms
async function generateBrandKeywordRecommendationsForCountry(country: string, connectionUrl: string): Promise<number> {
  const sqlClient = postgres(connectionUrl, { ssl: 'require' });
  ...
  try {
    ...
    for (const kw of keywordData) {
      ...
      await saveRecommendation({
        ...
        targeting: kw.targeting,
        match_type: kw.match_type,
        recommendation_type: 'brand_keyword_bid',
        old_value: currentBid,
        recommended_value: recommendedBid,
        pre_acos_t0: t0Acos !== null && t0Acos !== 999 ? t0Acos : undefined,
        pre_acos_30d: d30Acos !== null && d30Acos !== 999 ? d30Acos : undefined,
